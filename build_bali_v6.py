#!/usr/bin/env python3
"""
발리 여행 블로그 빌드 시스템 v6 — 대규모 개선판
- 924개 HTML 페이지 (11지역 × 6카테고리 × 14페이지)
- 각 페이지별 고유한: 제목, 메타 설명, 첫 문단, Q&A, 본문, 팁, 에피소드
- 네이버 Cue: 최적화 구조 (표, 리스트, 결론박스, Q&A)
- SEO 개선: 고유 메타 설명, OG 이미지, canonical URL
- 인덱스 페이지, 사이트맵, robots.txt 자동 생성
- 마이리얼트립 제휴 링크
"""
...
# 지역별 대표 키워드 (검색 의도 반영)
AREA_SEARCH_INTENT = {
    "우붓": {"popular": "우붓 맛집", "season": "우붓 라이스테라스", "tip": "우붓 원숭이숲"},
    "스미냑": {"popular": "스미냑 비치클럽", "season": "스미냑 선셋", "tip": "스미냑 마사지"},
    "꾸따": {"popular": "꾸따 서핑", "season": "꾸따 해변", "tip": "꾸따 가성비 숙소"},
    "사누르": {"popular": "사누르 일출", "season": "사누르 자전거", "tip": "사누르 맛집"},
    "누사두아": {"popular": "누사두아 리조트", "season": "누사두아 해변", "tip": "누사두아 가족여행"},
    "울루와뚜": {"popular": "울루와뚜 사원", "season": "울루와뚜 서핑", "tip": "울루와뚜 비치클럽"},
    "짠디다사": {"popular": "짠디다사 다이빙", "season": "짠디다사 해변", "tip": "짠디다사 스노클링"},
    "로비나": {"popular": "로비나 돌고래", "season": "로비나 해변", "tip": "로비나 온천"},
    "킨타마니": {"popular": "킨타마니 화산", "season": "킨타마니 일출", "tip": "킨타마니 트레킹"},
    "타나롯": {"popular": "타나롯 사원", "season": "타나롯 일몰", "tip": "타나롯 포토스팟"},
    "베두굴": {"popular": "베두굴 호수", "season": "베두굴 사원", "tip": "베두굴 농시장"},
}

CATEGORIES = {
    "food": {"name": "음식/맛집", "icon": "🍜", "desc": "맛집 탐방", "en": "food", "kw": "맛집 추천"},
    "culture": {"name": "문화/사원", "icon": "🛕", "desc": "문화 체험", "en": "culture", "kw": "사원 투어"},
    "beach": {"name": "해변/서핑", "icon": "🏖️", "desc": "해변 액티비티", "en": "beach", "kw": "해변 추천"},
    "nature": {"name": "자연/모험", "icon": "🌿", "desc": "자연 탐방", "en": "nature", "kw": "자연 명소"},
    "shopping": {"name": "쇼핑/마사지", "icon": "🛍️", "desc": "쇼핑 & 힐링", "en": "shopping", "kw": "쇼핑 추천"},
    "transport": {"name": "여행/교통", "icon": "🚗", "desc": "이동 정보", "en": "transport", "kw": "교통 정보"},
}

from build_bali import BALI_DATA, CATEGORY_EXTRA_CONTENT

# ============================================================
# 고유 콘텐츠 생성 엔진 (v6 대폭 강화)
# ============================================================
...
# 제목 템플릿 v6 — 30가지 변형 (지역별 검색의도 반영)
TITLE_TEMPLATES_V6 = [
    "{area} {cat_name} 추천 — {year} 최신 가이드",
    "{area} {cat_name} 완벽 정리 — 가격·팁·후기",
    "{area} 여행 {cat_name} — 현지인이 추천하는 진짜 정보",
    "{area} {cat_name} 베스트 — {year} 실제 후기",
    "{area} 자유여행 {cat_name} — 놓치면 후회하는 꿀팁",
    "{area} {cat_name} 가이드 — 10년차 블로거 추천",
    "{area} {cat_name} 총정리 — 비용·위치·팁 비교",
    "{area} {cat_name} 탐방기 — 직접 다녀온 솔직 리뷰",
    "{area} {cat_name} 추천 리스트 — 예산별 BEST",
    "{area} {cat_name} 숨은 명소 — 현지인만 아는 곳",
    "{area} {cat_name} 맛있는 코스 — 하루 일정 추천",
    "{area} {cat_name} 포토스팟 — 인생샷 찍는 법",
    "{area} {cat_name} 비용 절약 — 가성비 최적화 팁",
    "{area} {cat_name} 초보 가이드 — 처음 방문자를 위해",
    "{year} {area} {cat_name} — 최신 가격 정보",
    "{area} {cat_name} 코스 추천 — {days}일 일정",
    "{area} {cat_name} 비교 — 어디가 좋을까?",
    "{area} {cat_name} 후기 — 솔직하게 말하면",
    "{area} {search_kw} — {year}년 {season} 추천",
    "{area} {cat_name} 맛집 리스트 — 현지인 PICK",
    "{area} {cat_name} 완벽 가이드 — 이것만 보면 끝",
    "{area} 여행 필수 — {cat_name} 정보 총정리",
    "{area} {cat_name} 추천 — 가격부터 팁까지",
    "{area} {cat_name} BEST — {year} 업데이트",
    "{area} {cat_name} 가이드 — 비용 절약 꿀팁",
    "{area} {cat_name} 추천 코스 — 현지 가이드",
    "{area} {cat_name} 정보 — 블로그에 없는 진짜 후기",
    "{area} {cat_name} 정리 — {year} 최신",
    "{area} {cat_name} 추천 — 예산별 정리",
    "{area} {cat_name} — {year} 자유여행 가이드",
]

# 메타 설명 템플릿 v6 — 20가지 (고유화 핵심)
META_DESC_TEMPLATES = [
    "{area} {cat_name} 여행 정보. {highlight1}. {highlight2}. {year}년 기준 최신 후기.",
    "{area} {cat_name} 추천 가이드. {highlight1}, {highlight2}까지. 가격 비교와 실전 팁.",
    "{area} 자유여행 {cat_name} 완벽 정리. {highlight1}. {year}년 실제 후기 기반.",
    "{area} {cat_name} — {highlight1}부터 {highlight2}까지. 현지인이 추천하는 진짜 정보.",
    "{year}년 {area} {cat_name} 최신 가이드. {highlight1}, {highlight2}. 비용 절약 팁 포함.",
    "{area} 여행 {cat_name} 정보. {highlight1} 추천. {highlight2} 가격 비교.",
    "{area} {cat_name} 베스트 추천. {highlight1}과 {highlight2} 비교. {year} 최신.",
    "{area} {cat_name} 완벽 가이드. {highlight1}, {highlight2}. 숨은 명소까지 공개.",
    "{area} {cat_name} — {year} 최신 정보. {highlight1} 추천. 가격·팁·후기 정리.",
    "{area} 자유여행 필수 정보. {cat_name} 추천. {highlight1}. {highlight2}.",
    "{area} {cat_name} 총정리. {highlight1}부터 {highlight2}까지. 예산별 추천.",
    "{area} {cat_name} 탐방기. {highlight1} 후기. {year}년 기준 가격 정보.",
    "{area} {cat_name} 가이드. {highlight1} 추천. {highlight2} 꿀팁.",
    "{area} {cat_name} — {year} 업데이트. {highlight1}, {highlight2} 정보.",
    "{area} {cat_name} 추천 리스트. {highlight1}. {highlight2}. 현지인 PICK.",
    "{area} 여행 {cat_name} — {highlight1}. {highlight2}. {year} 최신.",
    "{area} {cat_name} 정보 총정리. {highlight1} 추천. 가격 비교 포함.",
    "{area} {cat_name} — 현지인이 알려주는 진짜 정보. {highlight1}. {highlight2}.",
    "{area} {cat_name} 완벽 분석. {highlight1}과 {highlight2} 비교. {year} 후기.",
    "{area} {cat_name} 추천 — {year} 자유여행 가이드. {highlight1}. {highlight2}.",
]
...
INTRO_TEMPLATES = [
    ...
    "{area} {cat_name} {year}년 최신 정보입니다. {conclusion}. {var1}부터 {var3}까지 업데이트했습니다.",
...
def get_first_available_image(area, category):
    """대표 이미지 1장 경로 반환 (OG 이미지용)"""
    mapping = {}
    for f in [MAPPING_FILE, BASE_DIR / "image_mapping_v2.json", BASE_DIR / "image_mapping.json"]:
        if f.exists():
            try:
    ...
    search_dir = OUTPUT_IMAGES / area / category
    if search_dir.exists():
        files = sorted([f.name for f in search_dir.iterdir() if f.suffix.lower() in ('.jpg', '.png', '.webp')])
        if files:
...
def generate_unique_title(area, category, page_idx):
    """고유 제목 생성 v6 — 30가지 변형"""
    cat_info = CATEGORIES[category]
    area_intent = AREA_SEARCH_INTENT.get(area, {})
    rng = random.Random(hash(f"{area}_{category}_{page_idx}_title_v6"))
    
    # page_idx에 따라 다른 템플릿 사용 (30개 중 14개 선택)
    template_idx = (page_idx * 2 + rng.randint(0, 1)) % len(TITLE_TEMPLATES_V6)
    template = TITLE_TEMPLATES_V6[template_idx]
    
    title = template.format(
        area=area,
        cat_name=cat_info["name"],
        year=CURRENT_YEAR,
        days=rng.choice(["1", "2", "3", "4"]),
        search_kw=area_intent.get("popular", cat_info["kw"]),
        season=rng.choice(["봄", "여름", "가을", "겨울", "건기", "우기"]),
    )
    return title


def generate_unique_meta_desc(area, category, page_idx, data):
    """고유 메타 설명 생성 v6 — 20가지 변형"""
    cat_info = CATEGORIES[category]
    rng = random.Random(hash(f"{area}_{category}_{page_idx}_meta_v6"))
    ...
    highlight_options = []
    if spots:
        highlight_options.append(f"{spots[0]} 추천")
        if len(spots) > 1:
            highlight_options.append(f"{spots[1]} 방문")
    if food:
        highlight_options.append(f"{food[0].get('name', '현지 맛집')} 후기")
    if hidden:
        highlight_options.append(f"숨은 명소 공개")
    highlight_options.extend([
        f"가격 비교",
        f"실전 팁",
        f"교통 정보",
        f"숙소 추천",
    ...
    h1 = rng.choice(highlight_options[:5])
    h2 = rng.choice(highlight_options[2:7])
    
    template_idx = (page_idx * 3 + rng.randint(0, 2)) % len(META_DESC_TEMPLATES)
    template = META_DESC_TEMPLATES[template_idx]
...
def generate_unique_intro(area, category, page_idx, data):
    ...
    conditions = [
        "첫 방문이라면", "가성비를 중시한다면", "사진을 많이 찍고 싶다면",
        "가족과 함께라면", "혼자 여행한다면", "비수기에 방문한다면",
        "로컬 음식을 즐기고 싶다면", "역사와 문화에 관심이 있다면",
    ]
...
def generate_unique_content_body(area, category, page_idx, data):
    ...
    angle_map = {
        "실전_후기": (f'📋 {area} {cat_info["name"]} 실전 후기', f'직접 다녀온 기준으로 {area}의 {cat_info["name"]} 정보를 정리했습니다. 가격, 위치, 팁까지 모두 실제 경험담입니다.'),
        "가성비_최적화": (f'💰 {area} {cat_info["name"]} 가성비 최적화 가이드', f'예산을 아끼면서도 {area}의 {cat_info["name"]}을(를) 제대로 즐기는 방법을 정리했습니다.'),
        "숨은_명소": (f'💎 {area} {cat_info["name"]} 숨은 명소', f'관광객이 잘 모르는 {area}의 {cat_info["name"]} 명소를 공개합니다.'),
        "첫방문_가이드": (f'🔰 {area} {cat_info["name"]} 첫 방문 가이드', f'처음 {area}를 방문하시는 분들을 위해 {cat_info["name"]} 정보를 쉽게 정리했습니다.'),
        "커플_가족": (f'💑 {area} {cat_info["name"]} — 커플/가족 추천', f'커플 여행, 가족 여행에 맞는 {area}의 {cat_info["name"]} 추천 코스입니다.'),
        "사진_포토스팟": (f'📸 {area} {cat_info["name"]} 포토스팟 가이드', f'인스타 감성 사진을 찍을 수 있는 {area}의 {cat_info["name"]} 명소를 정리했습니다.'),
        "혼자_여행": (f'🎒 {area} 혼자 여행 — {cat_info["name"]} 가이드', f'혼자서도 안전하고 즐겁게 {area}의 {cat_info["name"]}을(를) 즐기는 방법입니다.'),
        "맛집_투어": (f'🍽️ {area} 맛집 투어 코스', f'하루 3끼를 {area}의 맛집에서 해결하는 투어 코스를 추천합니다.'),
        "액티비티": (f'🎯 {area} {cat_info["name"]} 액티비티 추천', f'체험과 활동 중심으로 {area}의 {cat_info["name"]}을(를) 즐기는 방법입니다.'),
        "힐링_휴양": (f'🧘 {area} 힐링 여행 — {cat_info["name"]}', f'휴식과 힐링 중심으로 {area}의 {cat_info["name"]}을(를) 즐기는 코스입니다.'),
        "문화_역사": (f'🏛️ {area} 문화와 역사 — {cat_info["name"]}', f'{area}의 문화적 배경과 역사를 이해하면서 {cat_info["name"]}을(를) 즐기는 방법입니다.'),
        "일몰_일출": (f'🌅 {area} 시간대별 명소 — {cat_info["name"]}', f'일몰, 일출, 골든아워에 {area}의 {cat_info["name"]}을(를) 제대로 즐기는 방법입니다.'),
        "비수기_여행": (f'🌧️ {area} 비수기 여행의 장점 — {cat_info["name"]}', f'비수기(11~3월)에 {area}의 {cat_info["name"]}을(를) 즐기면 좋은 이유를 정리했습니다.'),
        "로컬_체험": (f'🏠 {area} 로컬 체험 — {cat_info["name"]}', f'현지인처럼 {area}의 {cat_info["name"]}을(를) 즐기는 방법을 알려드립니다.'),
    }
    
    title, desc = angle_map.get(angle, angle_map["실전_후기"])
    html += f'<h2>{title}</h2>\n<p>{desc}</p>\n'
...
def generate_unique_anecdote(area, category, page_idx, data):
    ...
    result = template.format(
        ...
        observation=rng.choice([
            ...
            "한국인 여행자가 아직 많지 않아 조용하게 즐길 수 있었습니다",
        ]),
        visit_count=rng.choice(["3", "5", "7", "10"]),
        ...
        surprising_fact=rng.choice([
            "입장료가 무료인 곳이 많다는 것",
            "로컬 음식 가격이 한국의 1/5 수준이라는 것",
            "자연 명소가 관광지화되지 않고 보존되어 있다는 것",
            "현지인들이 한국 문화에 관심이 많다는 것",
        ]),
        insight=rng.choice([
            "갈 때마다 새로운 명소가 생기고 있어요",
            "비수기가 오히려 여행하기 더 좋습니다",
            "로컬 맛집이 관광지 맛집보다 훨씬 낫습니다",
            "10년 전과 비교하면 인프라가 크게 발전했어요",
        ]),
        ...
        question=rng.choice([
            "여기서 가장 맛있는 집이 어디예요?", "사진 찍기 좋은 곳은요?",
            "비수기에 오면 사람이 적나요?", "가성비 숙소 추천해주세요",
        ]),
        answer=rng.choice([
            "로컬 와룽이 최고예요", "일몰 시간에 오세요",
            "비수기가 훨씬 좋아요", "게스트하우스도 충분합니다",
        ]),
    )
...
def generate_related_links(area, category, page_idx):
    ...
    if not related:
        related = random.sample([a for a in AREAS if a != area], 3)
    
    rng_link = random.Random(hash(f"{area}_{category}_{page_idx}_links"))
    related = related[:3]
    
    html = f'<h2>🔗 관련 지역 추천</h2>\n<div style="display:flex;flex-wrap:wrap;gap:10px;margin:12px 0">\n'
    for ra in related:
        rpage = rng_link.randint(1, 14)
...
def generate_html(area, category, page_idx):
    """단일 HTML 페이지 생성 (v6 고유 콘텐츠)"""
    data = BALI_DATA.get(area, {})
    cat_info = CATEGORIES[category]
    
    # 고유 제목 (v6)
    title = generate_unique_title(area, category, page_idx)
    
    # 고유 메타 설명 (v6)
    meta_desc = generate_unique_meta_desc(area, category, page_idx, data)
    ...
    # OG 이미지 (실제 이미지 경로 사용)
    og_image = get_first_available_image(area, category)
    ...
    for area in AREAS:
        for category in CATEGORIES:
            for page_idx in range(14):
                ...
                date = datetime(2026, 4, 1) + timedelta(days=page_idx % 30)
                urls.append(f"""<url>
<loc>{url}</loc>
...
{"".join(urls)}
    ...
    sitemap_path = OUTPUT_HTML.parent / "sitemap.xml"
    sitemap_path.write_text(xml, encoding='utf-8')
    print(f"📄 sitemap.xml 생성 완료 ({len(urls)} URLs)")
...
Allow: /
'''
    robots_path = OUTPUT_HTML.parent / "robots.txt"
...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<meta name="robots" content="index, follow">
<title>발리 여행 블로그 — {CURRENT_YEAR} 자유여행 완벽 가이드</title>
<meta name="description" content="발리 자유여행 완벽 가이드. 우붓, 스미냑, 꾸따 등 11개 지역별 맛집, 해변, 문화, 쇼핑 정보. {CURRENT_YEAR} 최신 가격 비교와 실전 팁.">
<link rel="canonical" href="{SITE_URL}/">
<meta property="og:title" content="발리 여행 블로그 — {CURRENT_YEAR} 자유여행 완벽 가이드">
<meta property="og:description" content="발리 자유여행 완벽 가이드. 11개 지역별 맛집, 해변, 문화, 쇼핑 정보.">
<meta property="og:type" content="website">
<meta property="og:url" content="{SITE_URL}/">
<style>
:root {{ --primary: #FF6B35; --bg: #FAFAFA; --text: #1A1A2E; --text-light: #666; --card-bg: #FFFFFF; --shadow: 0 2px 8px rgba(0,0,0,0.08); }}
* {{ margin: 0; padding: 0; box-sizing: border-box; }}
body {{ font-family: 'Pretendard', -apple-system, BlinkMacSystemFont, sans-serif; background: var(--bg); color: var(--text); line-height: 1.8; }}
header {{ background: linear-gradient(135deg, #FF6B35, #FF8C61); color: white; padding: 60px 20px; text-align: center; }}
header h1 {{ font-size: 2.2rem; margin-bottom: 10px; }}
header p {{ opacity: 0.9; font-size: 1.1rem; }}
.container {{ max-width: 1000px; margin: 0 auto; padding: 20px; }}
.area-grid {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 20px; margin: 30px 0; }}
.area-card {{ background: var(--card-bg); border-radius: 12px; padding: 24px; box-shadow: var(--shadow); transition: transform 0.2s; }}
.area-card:hover {{ transform: translateY(-4px); }}
.area-card h2 {{ color: var(--primary); font-size: 1.3rem; margin-bottom: 12px; }}
.area-card p {{ color: var(--text-light); font-size: 0.95rem; margin-bottom: 16px; }}
.cat-links {{ display: flex; flex-wrap: wrap; gap: 8px; }}
.cat-links a {{ display: inline-block; padding: 6px 14px; background: #F0F0F0; color: var(--text); border-radius: 20px; text-decoration: none; font-size: 0.85rem; transition: background 0.2s; }}
.cat-links a:hover {{ background: var(--primary); color: white; }}
footer {{ text-align: center; padding: 40px 20px; color: var(--text-light); font-size: 0.85rem; border-top: 1px solid #E0E0E0; margin-top: 40px; }}
</style>
</head>
<body>
<header>
<h1>🌴 발리 여행 블로그</h1>
<p>{CURRENT_YEAR} 자유여행 완벽 가이드 — 11개 지역 × 6개 카테고리</p>
</header>
<div class="container">
<p style="margin:20px 0;font-size:1.05em;color:#555">발리 여행에 필요한 모든 정보를 지역별, 카테고리별로 정리했습니다. 가격 비교, 숨은 명소, 실전 팁까지 — {CURRENT_YEAR} 최신 정보입니다.</p>
<div class="area-grid">'''

    for area in AREAS:
        data = BALI_DATA.get(area, {})
        desc = data.get("description", f"{area} 여행 정보")
        html += f'''
<div class="area-card">
<h2>{area}</h2>
<p>{desc}</p>
<div class="cat-links">'''
        for cat_key, cat_info in CATEGORIES.items():
            html += f'<a href="/{area}/{cat_key}/001.html">{cat_info["icon"]} {cat_info["name"]}</a>'
        html += '</div></div>'
...
<p style="margin-top:8px">이 글은 마이리얼트립 파트너스 활동의 일환으로, 이에 따른 일정액의 수수료를 제공받습니다.</p>
    ...
    index_path = OUTPUT_HTML / "index.html"
...
def main():
    print("=" * 60)
    print("🏗️  발리 여행 블로그 빌드 시스템 v6 (대폭 개선)")
    print("=" * 60)
    ...
    print(f"\n📁 위치: {OUTPUT_HTML}")
    print(f"📄 sitemap.xml, robots.txt, index.html 생성 완료")
